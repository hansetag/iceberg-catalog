...
(
    view_version_unique_id uuid        not null primary key default uuid_generate_v1mc(),
    view_id                uuid        not null REFERENCES view (view_id) ON DELETE CASCADE,
...
(
    ...
    view_version_unique_id uuid not null REFERENCES view_version (view_version_unique_id) ON DELETE CASCADE,
    key                    text not null,
    value                  text not null
);
