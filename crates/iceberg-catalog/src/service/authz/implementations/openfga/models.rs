...
lazy_static::lazy_static! {
    static ref MODEL_V1_JSON: serde_json::Value = serde_json::from_str(include_str!("../../../../../../../authz/openfga/collaboration_model/v1/schema.json")).expect("Failed to parse OpenFGA model V1 as JSON");
...
impl CollaborationModels {
    #[must_use]
    pub fn get_model(&self, version: &CollaborationModelVersion) -> &AuthorizationModel {
        match version {
            CollaborationModelVersion::V1 => &self.v1,
        }
    }
}
...
pub enum CollaborationModelVersion {
    V1,
}

impl CollaborationModelVersion {
    #[must_use]
    pub fn latest() -> Self {
...
mod test {
    ...
    #[test]
    fn test_load_collaboration_model() {
        let _model = MODEL.get_model(&CollaborationModelVersion::V1);
    }
    ...
    fn test_load_ser_de_roundtrip() {
        ...
        println!("{}", serde_json::to_value(&ser_model).unwrap());
        assert_eq!(serde_json::to_value(ser_model).unwrap(), *MODEL_V1_JSON);
    }

    #[test]
    fn test_proto_roundtrip() {
        let proto = MODEL.get_model(&CollaborationModelVersion::V1).clone();
