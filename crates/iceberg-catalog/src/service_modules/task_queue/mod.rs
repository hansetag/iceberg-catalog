...
use crate::service_modules::{CatalogBackend, SecretStore};
...
mod test {
    ...
    mod minio {
        ...
        use crate::service_modules::task_queue::tabular_expiration_queue::TabularExpirationInput;
        use crate::service_modules::task_queue::{TaskQueue, TaskQueueConfig};
        use crate::service_modules::{CatalogBackend, ListFlags, SecretStore};
        use sqlx::PgPool;
        use std::sync::Arc;
        ...
        async fn test_queue_expiration_queue_task(pool: PgPool) {
            ...
            let rw =
                crate::service_modules::catalog_backends::implementations::postgres::ReadWrite::from_pools(
                    pool.clone(),
                    pool.clone(),
                );
            let expiration_queue = Arc::new(
                crate::service_modules::catalog_backends::implementations::postgres::task_queues::TabularExpirationQueue::from_config(
                    rw.clone(),
                    config,
                )
                .unwrap(),
            ...
            let purge_queue = Arc::new(
                crate::service_modules::catalog_backends::implementations::postgres::task_queues::TabularPurgeQueue::from_config(
                    rw.clone(),
                    config,
                )
                .unwrap(),
            ...
            let queues = crate::service_modules::task_queue::TaskQueues::new(
                expiration_queue.clone(),
                purge_queue,
            );
            let secrets = crate::service_modules::catalog_backends::implementations::postgres::SecretsState::from_pools(
                pool.clone(),
                pool,
            );
