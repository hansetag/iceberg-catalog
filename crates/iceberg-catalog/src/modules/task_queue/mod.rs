...
mod test {
    ...
    mod minio {
        use crate::modules::catalog_backends::implementations::postgres::tabular::table::tests::initialize_table;
        ...
        use crate::modules::{CatalogBackend, ListFlags, SecretStore};
        ...
        async fn test_queue_expiration_queue_task(pool: PgPool) {
            ...
            let rw =
                crate::modules::catalog_backends::implementations::postgres::ReadWrite::from_pools(
                    pool.clone(),
                    pool.clone(),
                );
            let expiration_queue = Arc::new(
                crate::modules::catalog_backends::implementations::postgres::task_queues::TabularExpirationQueue::from_config(
                    rw.clone(),
                    config,
                )
                .unwrap(),
            ...
            let purge_queue = Arc::new(
                crate::modules::catalog_backends::implementations::postgres::task_queues::TabularPurgeQueue::from_config(
                    rw.clone(),
                    config,
                )
                .unwrap(),
            ...
            let secrets = crate::modules::catalog_backends::implementations::postgres::SecretsState::from_pools(
                pool.clone(),
                pool,
            );
