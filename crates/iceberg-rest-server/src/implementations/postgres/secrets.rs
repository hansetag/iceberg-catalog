...
impl SecretStore<SecretsState> for Server {
...
#[cfg(test)]
mod tests {
    use crate::service::storage::S3Credential;
    ...
    async fn test_write_read_secret(pool: sqlx::PgPool) {
        ...
        let secret = S3Credential::AccessKey {
            aws_access_key_id: "my access key".to_string(),
            aws_secret_access_key: "my secret key".to_string(),
        };

        let secret_id = Server::create_secret(state.clone(), secret.clone())
            .await
            .unwrap();

        let read_secret = Server::get_secret_by_id::<S3Credential>(state.clone(), &secret_id)
            .await
            .unwrap();
